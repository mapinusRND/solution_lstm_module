...
def load_data_from_db(tablename, dateColumn, studyColumns):
    ...
    try:
        ...
        AND TO_CHAR(time_point, 'MM-DD') NOT IN (
...
def run_single_experiment(experiment_config, experiment_index):
    ...
    if result['status'] == 'success':
        ...
        save_success = save_experiment_to_db(
            result, 
            experiment_config,
            is_new_model=result.get('is_new_model', False)  # ✅ 전달
        )
...
def lstmFinance(lstmData, config):
    ...
    def mean_absolute_percentage_error(y_true, y_pred):
        ...
        mape_day = np.mean(np.abs((y_pred[mask] - y_true[mask]) / y_true[mask])) * 100
        # print("eps : ",eps);
        print("y_true : ",y_true);
        print("y_pred : ",y_pred);
        print("mask : ",mask);
        ...
        # mask = y_true != 0
        # # if np.sum(mask) == 0:
        # if np.sum(mask) <= 20 and np.sum(mask) == 0:
        #     return 999.0
...
if __name__ == "__main__":
    ...
    print("   5. PostgreSQL DB에 실험 결과 자동 저장")  # 추가
    print("=" * 60)
